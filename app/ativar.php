        ...
        tailwind.config = {
            ...
            theme: {
                extend: {
                    ...
                    "fontFamily": { "label-caps": ["Inter", "ui-sans-serif", "system-ui", "sans-serif"], "label-bold": ["Inter", "ui-sans-serif", "system-ui", "sans-serif"], "body-md": ["Inter", "ui-sans-serif", "system-ui", "sans-serif"], "body-lg": ["Inter", "ui-sans-serif", "system-ui", "sans-serif"], "headline-sm": ["Inter", "ui-sans-serif", "system-ui", "sans-serif"], "headline-md": ["Inter", "ui-sans-serif", "system-ui", "sans-serif"], "display-lg": ["Inter", "ui-sans-serif", "system-ui", "sans-serif"] },
                    "fontSize": {
                        "label-caps": ["11px", { "lineHeight": "1", "letterSpacing": "0.06em", "fontWeight": "600" }], "label-bold": ["13px", { "lineHeight": "1", "letterSpacing": "0.01em", "fontWeight": "700" }], "body-md": ["15px", { "lineHeight": "1.45", "fontWeight": "400" }], "body-lg": ["17px", { "lineHeight": "1.5", "fontWeight": "400" }], "headline-sm": ["18px", { "lineHeight": "1.3", "letterSpacing": "-0.01em", "fontWeight": "600" }], "headline-md": ["22px", { "lineHeight": "1.25", "letterSpacing": "-0.02em", "fontWeight": "700" }], "display-lg": ["30px", { "lineHeight": "1.15", "letterSpacing": "-0.03em", "fontWeight": "700" }]
                    }
                },
            },
        }
...
<style>
        body { background-color: #f8f9fa; font-family: 'Inter', 'Manrope', sans-serif; color: #191c1d; }
...
<div class="flex items-center gap-3 border border-outline-variant/60 rounded-xl px-4 py-3 cursor-pointer transition-all plan-card <?php echo ($i === 0) ? 'border-primary bg-primary/5' : ''; ?> <?php echo $ehMensal ? 'border-amber-300 bg-amber-50/60' : ''; ?>" data-id="<?php echo (int)$pl['id']; ?>" data-valor="<?php echo htmlspecialchars($plValor); ?>" data-dias="<?php echo (int)$pl['dias']; ?>" data-nome="<?php echo htmlspecialchars($pl['nome']); ?>" onclick="selecionarPlano(this)">
<?php if ($ehMensal): ?>
...
<?php endif; ?>
<div class="flex-1">
<p class="font-body-md text-[14px] font-bold text-on-surface flex items-center gap-1.5">
...
<input id="cardSecurityCode" name="cardSecurityCode" type="text" autocomplete="off" inputmode="numeric" maxlength="4" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="123" required/>
</div>
</div>
<div class="grid grid-cols-2 gap-3">
<div>
<label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="cardPostalCode">CEP (endereço do titular)</label>
<input id="cardPostalCode" name="cardPostalCode" type="text" autocomplete="off" inputmode="numeric" value="<?php echo htmlspecialchars($cepUsuario); ?>" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="00000-000" required/>
</div>
<div>
<label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="cardAddressNumber">Número</label>
<input id="cardAddressNumber" name="cardAddressNumber" type="text" autocomplete="off" value="<?php echo htmlspecialchars($numEndereco); ?>" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="Nº" required/>
</div>
</div>
...
<script>
    ...
    <div id="modalMensal" class="hidden fixed inset-0 z-[120] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
    <div class="w-full max-w-md bg-surface-container-lowest rounded-2xl overflow-hidden shadow-2xl my-auto">
    <div class="p-5 border-b border-outline-variant/30 flex items-center justify-between bg-primary/5">
    <h2 class="font-headline-sm text-headline-sm text-on-surface">Plano Mensal</h2>
    <button type="button" onclick="fecharModalMensal()" class="text-on-surface-variant p-1 rounded-full hover:bg-surface-container-high transition-colors"><span class="material-symbols-outlined">close</span></button>
    </div>
    <div class="p-5">
    <div class="bg-secondary/10 border border-secondary/20 rounded-xl p-4 mb-4">
    ...
    <div class="flex bg-surface-container rounded-xl p-1 mb-4">
    <button type="button" class="flex-1 py-2.5 rounded-lg font-label-bold text-label-bold text-primary bg-surface-container-lowest shadow-sm flex items-center justify-center gap-1.5 transition-all" id="mTabPixBtn" onclick="mMostrarAba('pix')">
    <span class="material-symbols-outlined text-[18px]">qr_code_2</span> PIX
    </button>
    <button type="button" class="flex-1 py-2.5 rounded-lg font-label-bold text-label-bold text-on-surface-variant flex items-center justify-center gap-1.5 transition-all" id="mTabCardBtn" onclick="mMostrarAba('card')">
    <span class="material-symbols-outlined text-[18px]">credit_card</span> Cartão de crédito
    </button>
    </div>
    <!-- Modal: PIX Panel -->
    <div id="mPainelPix">
    <div class="w-40 h-40 bg-surface-container rounded-lg flex items-center justify-center mb-4 border-2 border-dashed border-outline-variant mx-auto" id="mqrBox">
    <span class="material-symbols-outlined text-[56px] text-outline opacity-40">qr_code_2</span>
    </div>
    <p class="font-body-md text-on-surface-variant mb-4 text-center">Valor a pagar: <b class="text-secondary" id="mPixValorText">R$ <?php echo number_format($valAdesaoGlobal, 2, ',', '.'); ?></b> (taxa de adesão)</p>
    <button type="button" id="mBtnGerarPix" class="w-full bg-secondary hover:bg-on-secondary-fixed-variant text-on-secondary font-headline-sm py-3 rounded-full shadow-lg active:scale-95 transition-all duration-200 flex items-center justify-center gap-2" onclick="mGerarPix()">
    <span class="material-symbols-outlined text-[20px]">qr_code_2</span> GERAR QR CODE PIX
    </button>
    <div class="w-full mt-3 hidden" id="mPixCodeBox">
    <p class="font-label-bold text-label-bold text-outline uppercase mb-1 text-left">Código PIX (copia e cola)</p>
    <div class="w-full bg-surface-container-low rounded-lg p-3 flex items-center gap-2">
    <p class="font-body-md text-[11px] text-on-surface-variant flex-1 break-all text-left" id="mPixCodeText"></p>
    <button type="button" class="text-primary font-label-bold text-[12px]" onclick="mCopiarPix()">COPIAR</button>
    </div>
    </div>
    <div class="w-full mt-3 hidden" id="mPixWaitBox">
    <p class="font-body-md text-on-surface-variant text-center mb-md">Aguardando pagamento...</p>
    </div>
    </div>
    <!-- Modal: Cartão Panel -->
    <div id="mPainelCard" class="hidden">
    <p class="font-body-md text-on-surface-variant mb-4 text-center">Pague com <b>cartão de crédito</b> para ativar seu cartão por <b id="mCardDiasText"><?php echo $planoMensal ? (int)$planoMensal['dias'] : '30'; ?></b> dias. Valor: <b id="mCardValorText">R$ <?php echo number_format($valAdesaoGlobal, 2, ',', '.'); ?></b>.</p>
    <?php if ($asaasOk): ?>
    <form id="mCardForm" class="space-y-3">
    <div>
    <label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="mCardholderName">Nome no cartão</label>
    <input id="mCardholderName" name="cardholderName" type="text" autocomplete="off" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="Nome impresso no cartão" required/>
    </div>
    <div>
    <label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="mCardNumber">Número do cartão</label>
    <input id="mCardNumber" name="number" type="text" autocomplete="off" inputmode="numeric" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="0000 0000 0000 0000" required/>
    </div>
    <div class="grid grid-cols-2 gap-3">
    <div>
    <label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="mCardExpirationDate">Vencimento</label>
    <input id="mCardExpirationDate" name="expiry" type="text" autocomplete="off" inputmode="numeric" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="MM/AA" required/>
    </div>
    <div>
    <label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="mCardSecurityCode">Código de segurança</label>
    <input id="mCardSecurityCode" name="ccc" type="text" autocomplete="off" inputmode="numeric" maxlength="4" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="123" required/>
    </div>
    </div>
    <div class="grid grid-cols-2 gap-3">
    <div>
    <label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="mCardPostalCode">CEP (endereço do titular)</label>
    <input id="mCardPostalCode" name="postal_code" type="text" autocomplete="off" inputmode="numeric" value="<?php echo htmlspecialchars($cepUsuario); ?>" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="00000-000" required/>
    </div>
    <div>
    <label class="font-label-bold text-label-bold text-outline uppercase mb-1 block" for="mCardAddressNumber">Número</label>
    <input id="mCardAddressNumber" name="address_number" type="text" autocomplete="off" value="<?php echo htmlspecialchars($numEndereco); ?>" class="w-full bg-surface-container-low rounded-lg px-3.5 py-2.5 text-[14px] text-on-surface placeholder:text-outline/60 focus:outline-none focus:ring-2 focus:ring-primary border border-outline-variant/40" placeholder="Nº" required/>
    </div>
    </div>
    ...
    <button type="submit" id="mCardPayBtn" class="w-full bg-primary hover:bg-primary-container text-on-primary font-headline-sm py-3.5 rounded-full shadow-lg active:scale-95 transition-all duration-200 flex items-center justify-center gap-2">
    ...
    <script>
        ...
        function abrirModalMensal() {
            ...
            if (qr) qr.innerHTML = '<span class="material-symbols-outlined text-[56px] text-outline opacity-40">qr_code_2</span>';
        ...
        function mMostrarAba(aba) {
            ...
            const on  = 'flex-1 py-2.5 rounded-lg font-label-bold text-label-bold text-primary bg-surface-container-lowest shadow-sm flex items-center justify-center gap-1.5 transition-all';
            const off = 'flex-1 py-2.5 rounded-lg font-label-bold text-label-bold text-on-surface-variant flex items-center justify-center gap-1.5 transition-all';
            ...
            fetch('verifica_pagamento.php?id=' + mPagamentoLocalId)
                ...
                .then(d => {
                    if (d.status === 'approved') {
                        ...
                        if (wb) wb.innerText = 'Pagamento confirmado!';
                        if (mPolling) clearInterval(mPolling);
                        mostrarModalAtivado();
                    } else if (d.status === 'rejected' || d.status === 'cancelled') {
                        if (mPolling) clearInterval(mPolling);
